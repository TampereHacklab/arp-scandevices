...
iprange="192.168.10.10-192.168.10.200"	# hacklab's dhcp range
...
while [ 0 ]; do
	...
	for i in ${!devices[*]}; do
		lastseen=$(expr $(date +%s) - ${devices[$i]})	# how long since lastseen?

		echo -n "$i  |  IP:${ipslist[$i]}  |  Lastseen:${lastseen}s  |  Status:${status[$i]} "

		if [ "${status[$i]}" == "" ]; then				# device responded to arp scan, changing status from away to near
			if (( $lastseen <= 2 )); then
				echo -n "Arrived!"
				status[$i]="Near"
